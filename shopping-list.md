...
Pocket-portable, captures other LoRa nodes within radio range. **MeshCore is the supported protocol for Heimdall today — Meshtastic exports need a parser nobody's written yet (see [[wardriving-hardware-survey]] §9).** If your goal is feeding the `meshcore_nodes` slot, buy hardware you can run MeshCore-compatible firmware on.
